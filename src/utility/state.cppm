...
public:
	...
	[[nodiscard]] auto get_previous() const -> T {
		return _previous;
	}

	[[nodiscard]] auto is(T state) const -> bool {
	...
	[[nodiscard]] auto entered(T state) const -> bool {
		return _previous != state && _current == state;
	}

	[[nodiscard]] auto exited(T state) const -> bool {
		return _previous == state && _current != state;
	}
