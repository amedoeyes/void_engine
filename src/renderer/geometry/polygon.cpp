...
static auto generate_polygon_attributes(const std::vector<glm::vec2>& vertices) -> Attributes {
...
auto generate_polygon_vertices(unsigned int sides) -> std::vector<glm::vec2> {
	...
	for (unsigned int i = 0; i < sides; ++i) {
		float angle = offset + static_cast<float>(i) * step;
...
auto create_polygon_mesh(const std::vector<glm::vec2>& vertices) -> Mesh {
	...
	Attributes attributes = generate_polygon_attributes(vertices);
