...
public:
	...
	template<typename T>
	void add_vertex_buffer(const std::vector<T>& data, buffer::Usage usage = buffer::Usage::static_draw) {
	...
	void set_indices(const std::vector<unsigned int>& data, buffer::Usage usage = buffer::Usage::static_draw);
	void set_primitive_type(renderer::PrimitiveType primitive);

	[[nodiscard]]
	auto get_vertex_array() -> VertexArray&;
	[[nodiscard]]
	auto get_vertex_array() const -> const VertexArray&;
	[[nodiscard]]
	auto get_element_buffer() -> buffer::ElementBuffer&;
	[[nodiscard]]
	auto get_element_buffer() const -> const buffer::ElementBuffer&;
	[[nodiscard]]
	auto get_primitive_type() const -> renderer::PrimitiveType;
	[[nodiscard]]
	auto get_count() const -> unsigned int;

	template<typename T>
	[[nodiscard]]
	auto get_vertex_buffer(unsigned int index) -> buffer::ArrayBuffer<T>& {
		return *static_cast<buffer::ArrayBuffer<T>*>(_vertex_buffers[index]);
	}

	template<typename T>
	[[nodiscard]]
	auto get_vertex_buffer(unsigned int index) const -> const buffer::ArrayBuffer<T>& {
		return *static_cast<buffer::ArrayBuffer<T>*>(_vertex_buffers[index]);
	}
