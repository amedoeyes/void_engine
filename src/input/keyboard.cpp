...
bool Keyboard::is_key_down(const Key& key) const {
	return get_state(key);
}

bool Keyboard::is_key_up(const Key& key) const {
	return !get_state(key);
}
...
bool Keyboard::get_state(const Key& key) const {
	auto it = _states.find(key);
	if (it == _states.end()) return false;
	return it->second;
}

void Keyboard::set_state(const Key& key, const bool& state) {
	_states[key] = state;
}

bool Keyboard::get_prev_state(const Key& key) const {
	auto it = _prev_states.find(key);
	if (it == _prev_states.end()) return false;
	return it->second;
}
void Keyboard::set_prev_state(const Key& key, const bool& state) {
...
void Keyboard::callback(
	...
	if (action == GLFW_PRESS) {
		kb->set_state(static_cast<Keyboard::Key>(key), true);
	} else if (action == GLFW_RELEASE) {
		kb->set_state(static_cast<Keyboard::Key>(key), false);
	}
}
