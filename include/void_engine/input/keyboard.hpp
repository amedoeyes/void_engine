...
class Keyboard {
	...
	bool get_state(const Key& key) const;
	void set_state(const Key& key, const bool& state);

	bool get_prev_state(const Key& key) const;
	void set_prev_state(const Key& key, const bool& state);
	...
	private:
	std::unordered_map<Key, bool> _states;
	std::unordered_map<Key, bool> _prev_states;
};
